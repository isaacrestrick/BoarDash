...
export class Stone extends Static {
    static getRequiredAssets() {
        return [
            ...
            { key: 'stone', path: 'stone.png', type: 'spritesheet', frameWidth: 16, frameHeight: 12 },
    ...
    static registerAnimations(scene: Phaser.Scene): void {
        ...
        if (!has('stone')) {
            scene.anims.create({ key: 'stone', frames: scene.anims.generateFrameNames('stone', { start: 0, end: 0 }), frameRate: 8, repeat: -1 });
        }
    ...
    constructor(scene: Phaser.Scene, x: number, y: number, scale = 2.5) {
        super(scene, x, y, { key: 'stone', scale });
        Stone.registerAnimations(scene);
        this.getSprite().play('stone', true);
    }
}